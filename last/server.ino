...
void loop() {
  ...
  // Check if client RFID display should be cleared (after 3 seconds)
  if (displayingClientRfid && (millis() - clientRfidDisplayTime >= 3000)) {
...
void handleClientRfid() {
  if (server.hasArg("plain")) {
    clientRfidTag = server.arg("plain");
    
    // Display client RFID tag on LCD for 3 seconds
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Client RFID Tag:");
    lcd.setCursor(0, 1);
    lcd.print(clientRfidTag);
    ...
    // Debug output
    Serial.print("Received client RFID tag: ");
    Serial.println(clientRfidTag);
    
    server.send(200, "text/plain", "Received RFID tag");
  } else {
    server.send(400, "text/plain", "No RFID tag received");
  }
}