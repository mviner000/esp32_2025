...
// Server information
const char* serverAddress = "192.168.4.1"; // Server ESP32 IP address
...
void checkRfid() {
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    // Send RFID tag to server if WiFi is connected
    if (wifiConnected) {
...
void sendRfidToServer(String rfidTag) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Send POST request with RFID tag
    int httpResponseCode = http.POST(rfidTag);
    
    // Debug output
    Serial.print("Sending RFID tag to server: ");
    Serial.println(rfidTag);
    ...
    if (httpResponseCode == 200) {
      ...
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Tag Sent to");
      lcd.setCursor(0, 1);
      lcd.print("Server: Success");
      delay(1000);
    }
  } else {
    Serial.println("WiFi not connected. Cannot send RFID tag to server.");
  }
}